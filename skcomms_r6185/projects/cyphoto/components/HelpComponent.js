	...
	methods : {
		...
		_show_PageLog : function() {
			if(skEnv.device.isTv) {
				...
				if(skEnv.device.vendor == "lg" && skFn.string.trim(skEnv.device.modelName) == "GLOBAL-PLAT3") return;
			} else {
				return;
			}
			...
			//변경된 로그 수집 파라메터 추가 2012-02-08 [16:09:29] @SoonyoungPark
			logUrl += "&app_id=" + skEnv.app.id;
			logUrl += "&agent_ver=" + skEnv.version.agent;
			logUrl += "&app_ver=" + skEnv.version.app;
			logUrl += "&st_tid=" + skFn.user.getUserTid();
		...
		show : function(arrMenuInfo, zoneStyle) {
			...
			if(skEnv.device.vendor == "lgcns") {
				...
				var HelpbarCss =	"#SceneHelpbar {position:absolute; left:0; top:627px; z-index:4; overflow:hidden; width:1280px; height:93px;}" +
									".helpbar {width:1158px; height:93px; padding:0 32px 0 90px;color:#fff; font-size:25px; background: url(../image/bg/bg_helpbar.png) repeat-x left top;}" +
									...
									".helpbar .list_icon li .key2 {width:124px; height:61px; padding:31px 0 0 36px;}" +
									".helpbar .list_icon li .key3 {width:145px; height:61px; padding:31px 0 0 36px;}" +
									".helpbar .list_icon li .key4 {width:164px; height:61px; padding:31px 0 0 36px;}" +
									".helpbar .list_icon li .key5 {width:186px; height:61px; padding:31px 0 0 36px;}" +
									".helpbar .list_icon li .focus2, .helpbar .list_icon li .focus3, .helpbar .list_icon li .focus4, .helpbar .list_icon li .focus5 {background:transparent url(../image/bg/bg_helpbar_focus.png) repeat-x left top;}" ;
			}
			else {
				// css 정의
				var HelpbarCss =	"#SceneHelpbar {position:absolute; left:0; top:627px; z-index:4; overflow:hidden; width:1280px; height:93px;}" +
		...
		*/
		changeShow : function(strMenuInfo) {
			...
			$("#helpbar_menu" + strMenuInfo.menuid).live("mouseover", function(event){
				// Helpbar에 focus가 되었을때 다른 focus가 생기면 해당 focus를 삭제하도록 수정함.(7.4 by sophia)
				//skTv.util.changeClass(this , overClassName);
				skTv.util.changeClass(strMenuInfo.menuid, overClassName);
			});
			
			$("#helpbar_menu" + strMenuInfo.menuid).live("mouseout", function(event){
				// Helpbar에 focus가 되었을때 다른 focus가 생기면 해당 focus를 삭제하도록 수정함.(7.4 by sophia)
				// skTv.util.changeClass(this , basicClassName);
				skTv.util.changeClass(strMenuInfo.menuid, basicClassName);
			});
